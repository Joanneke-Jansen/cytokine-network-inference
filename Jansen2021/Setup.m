...
%% We check whether a network configuration exists, with a lower relative AIC than AIC(S^{n_o}_{opt}),
...
disp('The model with edges:')
[~,n_o]=min(IL23_model_saved_chi2s.AIC);
selected_configuration=IL23_model_saved_chi2s.initial_model{n_o};
disp(selected_configuration)
disp(' has the lowest relative AIC')

for y=1:length(IL23_model_saved_chi2s.edges)
    if isequal(sort(IL23_model_saved_chi2s.edges{y}),sort(IL23_model_saved_chi2s.initial_model{n_o}))
        index=y;
