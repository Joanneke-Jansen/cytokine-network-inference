...
%dat = fun_iterateBootstrap(dat); JEJ: removed line
dat = fun_print(dat);
...
arFprintf([],'\n');
arFprintf(2,'       Maxima observed: %g (%g jumps within sorted chi2 are larger than %g)\n',dat.D,dat.D,dat.dchi2_thresh)
arFprintf(2,'               thereof: %i once, %i twice, %i three times, %i four times.\n',dat.f1,dat.f2,dat.f3,dat.f4)
arFprintf(2,'\n');
arFprintf(2,'      Estimates for the real number of maxima:\n');
fns = {'jackknife1','jackknife2','medial','chao1','chao2','ichao','bootstrap','n_bootstrap_iterate'}; %JEJ: removed line
fns = {'jackknife1','jackknife2','medial','chao1','chao2','ichao','bootstrap'}; %JEJ: added line
nest = NaN(size(fns));
for i=1:length(fns)
    nest(i) = dat.(fns{i});
    arFprintf(2,'  %20s: %g  \t(%g%s observed)\n',fns{i},nest(i),min(100,100*dat.D/dat.(fns{i})),'%');
end

%if(dat.p_bootstrap_iterate<0.9); JEJ: removed line
if 0; %JEJ: added line
arFprintf([],'\n   Note: A large number of optima can be erroneously suggested, \n   if optimization does not work or the fits did not converge.\n');
