...
function arFitLHS_Jansen2021(n, randomseed, log_fit_history, backup_save, use_cluster)
...
lb_for_LHS_initial_guess=ar.lb;%JEJ: added line
ub_for_LHS_initial_guess=ar.ub;%JEJ: added line
...
if ar.config.restartLHS ==1 && isempty(randomseed)
    %indnan = find(isnan(ar.chi2s)); %JEJ: removed line
    indnan = find(~isfinite(ar.chi2s)); %JEJ: added line
    counter = 1;
    ar.lhsRepeats = ones(size(ar.chi2s));
    %while ~isempty(indnan) && counter<10 %JEJ: removed line
    while ~isempty(indnan) %JEJ: added line
        counter = counter+1;
